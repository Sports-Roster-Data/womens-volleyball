...
rosters <- read_csv("rosters_2023_24.csv")
...
```{r}
positions_cleaned <- read_csv("https://raw.githubusercontent.com/dwillis/jour479x_fall_2022/main/data/vb_positions.csv") %>%
  select(-count)

# replace Ball State's position column with NAs
rosters <- rosters %>% mutate(position = ifelse(ncaa_id == 47, NA, position))
...
position_fixes <- rosters %>% left_join(positions_cleaned, by = "position") %>% filter(position == height)

rosters <- rosters %>% left_join(positions_cleaned, by = "position")

rosters %>% distinct(position_clean)

## Primary/Secondary positions based on order of position_clean

rosters <- rosters %>% mutate(position_full = position_clean)
rosters <- rosters %>% mutate(position_clean = na_if(position_clean, "N/A"))
rosters <- rosters %>% separate(position_clean, c('primary_position', 'secondary_position'), sep='/', extra='merge')
rosters <- rosters %>% mutate(primary_position = case_when((primary_position == "G") ~ "Guard", (primary_position == "C") ~ "Center", (primary_position == "F") ~ "Forward", TRUE ~ primary_position)) 
rosters <- rosters %>% mutate(secondary_position = case_when((secondary_position == "G") ~ "Guard", (secondary_position == "C") ~ "Center", (secondary_position == "F") ~ "Forward", TRUE ~ secondary_position))
rosters <- rosters %>% mutate(primary_position = case_when((primary_position == "G") ~ "Guard", (primary_position == "C") ~ "Center", (primary_position == "F") ~ "Forward", TRUE ~ primary_position))
rosters <- rosters %>% filter(!grepl("'", rosters$primary_position))
rosters <- rosters %>% filter(!grepl("5", rosters$primary_position))
rosters <- rosters %>% filter(!grepl("6", rosters$primary_position))
rosters <- rosters %>% filter(!grepl("'", rosters$secondary_position))
rosters <- rosters %>% filter(!grepl("5", rosters$secondary_position))
rosters <- rosters %>% filter(!grepl("6", rosters$secondary_position))
...
rosters <- rosters %>% 
  mutate(country = case_when(
    ...
    country %in% c('VICTORIA', 'SOUTH AUSTRALIA', 'WESTERN AUSTRALIA', 'TASMANIA', 'AUSTRAILA', 'AU', 'QUEENSLAND AUSTRALIA', 'AUSTRAILIA') ~ 'AUSTRALIA',
...
write_csv(rosters, "rosters_2023_24.csv", quote="all", na="")
```
