...
```{r}
domestic |> 
  left_join(us_cities, join_by(hometown, country)) |>
  
  #mutate(hometown_copy = hometown) |> separate(hometown_copy, into=c('city', 'state'), sep=",") |> mutate(city = trimws(city), state=trimws(state)) |> mutate(state = gsub('\\.','', state))
```
