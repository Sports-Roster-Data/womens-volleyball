...
rosters <- read_csv("rosters_2023-24_main.csv")

teams_without_rosters <- teams |> left_join(rosters, join_by(ncaa_id, team)) |> filter(is.na(name))

rosters2 <- read_csv("rosters_2023.csv")

teams_without_rosters2 <- teams |> left_join(rosters2, join_by(ncaa_id, team)) |> filter(is.na(name))

teams_without_rosters2 |> anti_join(teams_without_rosters, join_by(ncaa_id, team))
